...
router.get("/", (req, res) => {
    ...
    // if duration is not valide return an empty table 
    if (!req.query["duration-min"] || isNaN(durationMIN) || durationMIN < 0) {
        return res.json([]);
    }
    // filtred movies
...
router.post("/", (req, res) => {
    ...
    const nextId = defaultFilms.reduce((maxId, film) => (film.id > maxId ? film.id : maxId), 0) +
        1;
    // Create new movie
    const newFilm = {
        id: nextId,
